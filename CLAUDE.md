...
- `Attendance` — **presença na instrução** (`routes/attendance.ts`). Um registro por `(date, personNum+personNome, turmaId)` com `status` ∈ `PRESENTE | FALTA` e um flag `justificada` (só vale p/ FALTA). **Pontos disciplinares** (`domain.ts`: `PONTOS_INICIAL=120`, `calcularPontos`): cada militar começa com **120**; falta **não-justificada = -4**, **justificada = -2** (saldo cumulativo, todo o curso; só faltas da instrução contam — guarda não). A chamada só marca **P/F** (`POST /api/attendance` faz *replace* e **preserva** o flag `justificada` das faltas existentes p/ o monitor não apagar a classificação; pessoa sem registro = `PRESENTE`). Só **instrutor/Comandante** classificam: `PATCH /api/attendance/justificar` `{date,turmaId,num,nome,justificada}` (bloqueado p/ monitor via `podeClassificar` em `scope.ts`). `GET /api/attendance/historico` consolida P/F-não/F-just + saldo por pessoa num período; `GET /api/attendance/pontos` dá o saldo cumulativo (todo o curso) + detalhe das faltas. Migração única dos registros antigos `JUSTIFICADO`→`FALTA+justificada`: `npm run db:migrar-justificadas`. Escopado por turma como o resto.
...
- **Navegação/visão por papel** (`Sidebar.tsx` + `App.tsx`): a barra lateral agrupa as seções em VISÃO / OPERAÇÃO / CADASTRO; itens com `super:true` (`painel`, `usuarios`) só aparecem para o Comandante. **Horas de serviço** (guarda) e **Missões** (complementares) são itens **separados** no menu — dados distintos, sem soma entre si. **Pontos** (`PontosTab`, em OPERAÇÃO) mostra o saldo disciplinar (120 − faltas) por militar e é onde instrutor/Comandante justificam faltas (o monitor não vê esse controle, nem na chamada). **Auditoria** (em CADASTRO) aparece para todos, mas o servidor escopa por turma (Comandante vê tudo). O superadmin vê o **Painel do Comando** (`PainelTab`: cards por turma + gráficos de efetivo, horas e pontos médios por turma via `BarChart`) e uma barra de **filtro global de turma** (`FiltroTurma`, estado `turmaFoco`, "TODAS"/T1..T4) que filtra EFETIVO e SALVAS no client e define a turma padrão da geração. Instrutor/monitor não veem o filtro (já presos à sua turma). O **Calendário** (`CalendarioTab`) plota as escalas salvas num grid mensal — cada semana de guarda (terça→segunda) vira uma faixa colorida por turma (`corTurma`), clicável para abrir a escala.
