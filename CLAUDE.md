...
- `AuditLog` — **trilha de auditoria** (`server/src/audit.ts`). Cada requisição de escrita (POST/PUT/PATCH/DELETE) bem-sucedida vira um registro com *snapshot* do usuário (`username`/`role`/`turmaId`, sem FK p/ sobreviver à remoção da conta), `acao` (rótulo amigável), `method`/`rota` normalizada/`status`, `alvoId` (id na rota) e `detalhe` (resumo de campos seguros do corpo — **nunca** senha/CSV bruto). Gravado por um middleware global (`auditoriaMiddleware`, após `requireAuth`) que faz hook em `res.finish` — não toca nas rotas individuais. Consulta só pelo Comandante.
...
- Rotas em `server/src/routes/`: `turmas`, `dashboard`, `hours`, `people`, `schedule`, `history`, `aditamento`, `users`, `attendance` (presença), `missions` (horas complementares) — montadas sob `/api/*` em `server/src/index.ts`. `/api/users` e `/api/audit` (trilha de auditoria, `audit.ts`) são montadas atrás de `requireSuperadmin`; o `auditoriaMiddleware` roda logo após o `requireAuth` global e registra toda escrita. `GET /api/dashboard` (superadmin) devolve o resumo por turma (efetivo/ausentes/monitores/escalas/última + próxima turma do rodízio) para o **Painel do Comando**. `POST /api/people/atribuir-turma` (superadmin) move várias pessoas de turma de uma vez (seleção em massa na aba EFETIVO); `POST /api/people/importar-turmas` (superadmin) atribui turmas por CSV (`num;nome;turma`; turma por código `T1`, apelido `Caveira` ou número `1`; casa por `num+nome` ou só nome).
...
- **Navegação/visão por papel** (`Sidebar.tsx` + `App.tsx`): a barra lateral agrupa as seções em VISÃO / OPERAÇÃO / CADASTRO; itens com `super:true` (`painel`, `usuarios`, `auditoria`) só aparecem para o Comandante. A **Auditoria** (`AuditoriaTab`) lista as últimas ações registradas (filtro por texto). O superadmin vê o **Painel do Comando** (`PainelTab`: cards por turma + gráficos de efetivo e horas por turma via `BarChart`) e uma barra de **filtro global de turma** (`FiltroTurma`, estado `turmaFoco`, "TODAS"/T1..T4) que filtra EFETIVO e SALVAS no client e define a turma padrão da geração. Instrutor/monitor não veem o filtro (já presos à sua turma). O **Calendário** (`CalendarioTab`) plota as escalas salvas num grid mensal — cada semana de guarda (terça→segunda) vira uma faixa colorida por turma (`corTurma`), clicável para abrir a escala.
