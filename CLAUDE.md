...
- **Horas de serviço** (`HORAS` em `domain.ts`): permanência manhã/tarde = 6h, `Cmt Gd TG` e `Guardas do TG` (noite) = 12h. O relatório (`routes/hours.ts`) agrega por pessoa→turma e por mês (derivado de `startDate + dayIndex`).
